	...
	while (1) {
		...
		if (bms.state == NORMAL_OP || bms.state == ERROR_BMS) {
			if (bms.fault.charg_en == FAULTED
					...
					|| bms.fault.DOC == FAULTED || bms.fault.COC == FAULTED) {
				fault = FAULTED;
			}
...
//			WDAWG_PRIORITY, NULL);
	...
	xTaskCreate(task_BmsCanProcess, "BMS Can RX", BMSCAN_RX_STACK_SIZE, NULL,
			BMSCAN_TX_PRIORITY, NULL);

	xTaskCreate(task_txDcan, "transmit Dcan", DCAN_TX_STACK_SIZE, NULL,
			DCAN_TX_PRIORITY, NULL);

	xTaskCreate(task_DcanProcess, "Dcan Process", DCAN_RX_STACK_SIZE, NULL,
			DCAN_RX_PRIORITY, NULL);
...
void initBMSobject(flag_t mode) {
	...
	bms.macros.temp_avg = TEMP_AVG_INIT;
...
void task_bms_main() {
	...
	while (1) {
		...
		case INIT:
			...
			HAL_GPIO_WritePin(SDC_BMS_FAULT_GPIO_Port, SDC_BMS_FAULT_Pin,
					GPIO_PIN_RESET); //close SDC
			if (xSemaphoreTake(bms.state_sem, TIMEOUT) == pdPASS) {
...
Success_t send_faults() {
	...
	//macro faults
	msg.Data[0] = bitwise_or(FAULT_CHARGE_EN_SHIFT, FAULT_CHARGE_EN_MASK,
			bms.fault.COC);
	msg.Data[0] |= bitwise_or(FAULT_DISCHARGE_EN_SHIFT, FAULT_DISCHARGE_EN_MASK,
			bms.fault.DOC);
	msg.Data[0] |= bitwise_or(FAULT_OVERTEMP_SHIFT, FAULT_OVERTEMP_MASK,
			bms.fault.overtemp);
	msg.Data[0] |= bitwise_or(FAULT_OVERVOLT_SHIFT, FAULT_OVERVOLT_MASK,
			bms.fault.overvolt);
	msg.Data[0] |= bitwise_or(FAULT_UNDERVOLT_SHIFT, FAULT_UNDERVOLT_MASK,
			bms.fault.undervolt);
	msg.Data[0] |= bitwise_or(FAULT_UNDERTEMP_SHIFT, FAULT_UNDERTEMP_MASK,
			bms.fault.undertemp);

	//module specific faults
	for (i = 1; i < (NUM_SLAVES + 1); i++) {
		//low module
		msg.Data[i] = bitwise_or(FAULT_MODL_CON_SHIFT, FAULT_MODL_CON_MASK,
				bms.fault.slave[x].connected);
		...
		if (x + 1 < NUM_SLAVES) {
			...
			msg.Data[i] |= bitwise_or(FAULT_MODH_TEMP_SHIFT,
					FAULT_MODH_TEMP_MASK, bms.fault.slave[x + 1].temp_sens);
			msg.Data[i] |= bitwise_or(FAULT_MODH_VOLT_SHIFT,
					FAULT_MODH_VOLT_MASK, bms.fault.slave[x + 1].volt_sens);
		}
	}
...
Success_t temp_probe() {
	...
	bms.macros.temp_avg = temp_avg;

	//safety check
	//undervolt check
	if ((temp_low.val < bms.params.temp_low_lim)
			&& (temp_low.val != (int16_t) TEMP_LOW_IMPOS)) {
		if (xSemaphoreTake(bms.fault.sem, TIMEOUT) == pdPASS) {
			bms.fault.undertemp = FAULTED;
			xSemaphoreGive(bms.fault.sem);
		} else {
